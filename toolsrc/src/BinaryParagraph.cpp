...
namespace vcpkg
{
    ...
    BinaryParagraph::BinaryParagraph(std::unordered_map<std::string, std::string> fields)
    {
        ...
        this->spec = package_spec::from_name_and_triplet(name, target_triplet).get_or_throw(__LINE_INFO__);
        ...
        Checks::check_exit(__LINE_INFO__, multi_arch == "same", "Multi-Arch must be 'same' but was %s", multi_arch);
    ...
    {
        this->spec = package_spec::from_name_and_triplet(spgh.name, target_triplet).get_or_throw(__LINE_INFO__);
