...
namespace vcpkg::PostBuildLint
{
    ...
    BuildInfo BuildInfo::create(std::unordered_map<std::string, std::string> pgh)
    {
        ...
        Checks::check_exit(__LINE_INFO__, build_info.crt_linkage != LinkageType::NULLVALUE, "Invalid crt linkage type: [%s]", crt_linkage_as_string);
        ...
        Checks::check_exit(__LINE_INFO__, build_info.library_linkage != LinkageType::NULLVALUE, "Invalid library linkage type: [%s]", library_linkage_as_string);
        ...
        for (const std::unordered_map<std::string, std::string>::value_type& p : pgh)
        {
            ...
            Checks::check_exit(__LINE_INFO__, policy != BuildPolicies::NULLVALUE, "Unknown policy found: %s", p.first);
    ...
    BuildInfo read_build_info(const fs::path& filepath)
    {
        ...
        Checks::check_exit(__LINE_INFO__, pghs.size() == 1, "Invalid BUILD_INFO file for package");
