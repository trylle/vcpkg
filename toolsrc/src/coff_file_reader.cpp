...
namespace vcpkg::COFFFileReader
{
    ...
    static void read_and_verify_PE_signature(fstream& fs)
    {
        ...
        verify_equal_strings(__LINE_INFO__, PE_SIGNATURE, signature, PE_SIGNATURE_SIZE, "PE_SIGNATURE");
    ...
    struct archive_member_header
    {
        ...
        static archive_member_header read(fstream& fs)
        {
            ...
            if (ret.data[0] != '\0') // Due to freeglut. github issue #223
            {
                ...
                verify_equal_strings(__LINE_INFO__, HEADER_END, header_end.c_str(), HEADER_END_SIZE, "LIB HEADER_END");
            }
    ...
    struct import_header
    {
        ...
        static import_header read(fstream& fs)
        {
            ...
            Checks::check_exit(__LINE_INFO__, sig1 == SIG1, "Sig1 was incorrect. Expected %s but got %s", SIG1, sig1);
            ...
            Checks::check_exit(__LINE_INFO__, sig2 == SIG2, "Sig2 was incorrect. Expected %s but got %s", SIG2, sig2);
    ...
    static void read_and_verify_archive_file_signature(fstream& fs)
    {
        ...
        verify_equal_strings(__LINE_INFO__, FILE_START, file_start, FILE_START_SIZE, "LIB FILE_START");
    }

    dll_info read_dll(const fs::path& path)
    {
        std::fstream fs(path, std::ios::in | std::ios::binary | std::ios::ate);
        Checks::check_exit(__LINE_INFO__, fs.is_open(), "Could not open file %s for reading", path.generic_string());
    ...
    lib_info read_lib(const fs::path& path)
    {
        ...
        Checks::check_exit(__LINE_INFO__, first_linker_member_header.name().substr(0, 2) == "/ ", "Could not find proper first linker member");
        ...
        Checks::check_exit(__LINE_INFO__, second_linker_member_header.name().substr(0, 2) == "/ ", "Could not find proper second linker member");
