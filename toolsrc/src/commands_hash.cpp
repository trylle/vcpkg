...
namespace vcpkg::Commands::Hash
{
    static void do_file_hash(fs::path const& path, std::wstring const& hashType)
    {
        ...
        Checks::check_exit(__LINE_INFO__, start != std::string::npos, "Unexpected output format from command: %s", Strings::utf16_to_utf8(cmd_line));

        auto end = output.find_first_of("\r\n", start + 1);
        Checks::check_exit(__LINE_INFO__, end != std::string::npos, "Unexpected output format from command: %s", Strings::utf16_to_utf8(cmd_line));
