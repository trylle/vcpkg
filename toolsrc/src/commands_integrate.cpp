...
<package>
    ...
    static void integrate_install(const vcpkg_paths& paths)
    {
        ...
        if (should_install_system)
        {
            ...
            Checks::check_exit(__LINE_INFO__, fs::exists(system_wide_targets_file), "Error: failed to copy targets file to %s", system_wide_targets_file.string());
        }
    ...
    static void integrate_project(const vcpkg_paths& paths)
    {
        ...
        Checks::check_exit(__LINE_INFO__, exit_code == 0 && fs::exists(nuget_package), "Error: NuGet package creation failed");
